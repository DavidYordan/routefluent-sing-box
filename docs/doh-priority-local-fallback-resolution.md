...
日期：2026-07-11
状态：开发设计文档，当前 `v1.13.12-routefluent-anytls-client.3` 尚未实现本文能力
...
当 `rf-resolver-vendor-a` 的 DoH 恢复 healthy 后，该组内 `source=local_fallback` 的缓存项必须失效或不再优先命中。`local_only` 模式可以正常按 TTL 缓存，因为它不是临时 fallback。
...
6. 不修改 route selector、outbound selector、subscription parser 或代理协议实现。
